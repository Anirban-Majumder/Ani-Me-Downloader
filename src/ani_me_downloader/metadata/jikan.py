# coding: utf-8
"""Jikan v4 (unofficial MAL mirror) adapter. Fallback when AniList is down."""
import logging
...
class JikanUnavailable(Exception):
    """Jikan transport or HTTP failure."""
...
def _get(path: str, params: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    if not response.ok:
        raise JikanUnavailable(f"HTTP {response.status_code}: {response.text[:200]}")
    return response.json()


def _next_broadcast_unix(broadcast: dict[str, Any] | None) -> int:
    """Unix timestamp of next broadcast slot, or 0 if unknown."""
    if not broadcast:
        return 0
...
def _adapt_search_item(item: dict[str, Any]) -> dict[str, Any]:
    """Map a Jikan anime entry into the AniList media shape."""
...
def search(query: str, limit: int = 25) -> list[dict[str, Any]]:
    """Search MAL via Jikan. Entries shaped like AniList media."""
